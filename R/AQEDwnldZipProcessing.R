# basic graphing for data downloaded from sd cards eggs
# writes png files to the results directory
# writes Rdata files to the data folder
library(data.table)
library(plyr)
library(ggplot2)
library(car)
...
fileList <- unzip(paste(fileLocation,zipFile,sep="/"), list = TRUE)

# counters to keep track of specific egg types
v2a <- v2b <- v2c <- 0
# for loop over the number of files in the zip file.
for (i in 1:length(fileList)) {
  csvFileName <- fileList[i]
  #downloaded egg data doesn't include the egg serial number
  #   eggSerialNumber <- gsub(".csv", "", csvFileName)
  #   print(eggSerialNumber)
  csvFile <- paste(fileLocation,csvFileName,sep = "/")
...
#         "altitude_m"
  ...
  df.AQE$time_local <- as.POSIXct(df.AQE$time_local, format="%m/%d/%Y %H:%M:%S", tz = timezone)
  # commented out because the downloaded data from V2 eggs with firmware 2.0.5 doesn't have these)
  #   removeList <-
  #     c("latitude.deg.",
  #       "longitude.deg.",
  #       "altitude.m.")
  ...
  if (eggType %in% "AQEV2c") {
    ...
    p4 <- AQEplot(dataIn = df.AQE.min,
                  ...
                  colName = "pm_v")
    graphicFileName <-
      paste("results/",eggType, v2c, ".png", sep = "")
  }
  if (eggType %in% "AQEV2b") {
    ...
    p6 <- AQEplot(dataIn = df.AQE.min,
                  ...
                  colName = "o3_v")
    graphicFileName <-
      paste("results/",eggType, v2b, ".png", sep = "")
  }
  ...
  if (eggType %in% "AQEV2a") {
    ...
    p6 <- AQEplot(dataIn = df.AQE.min,
                  ...
                  colName = "co_v")
    graphicFileName <-
      paste("results/",eggType, v2a, ".png", sep = "")
  }
  ...
  gp1$widths[2:3] <-
    gp2$widths[2:3] <-
    gp3$widths[2:3] <- gp4$widths[2:3] <- maxWidth
  if (eggType %in% "AQEV2a" | eggType %in% "AQEV2b") {
  ...
  if (eggType %in% "AQEV2c") {
    ...
    grid.arrange(
      ...
      bottom = textGrob(eggType, gp = gpar(
        fontsize = 8, font = 3
      ))
    )
    dev.off()
