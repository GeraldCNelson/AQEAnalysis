...
AQE2aEggs <- c("1058978669","9544854","109431716","2122830268", "546125879","1203534876" ,"58540724")
# your location, find it at http://en.wikipedia.org/wiki/List_of_zoneinfo_time_zones
localTimeZone = "America/Denver"

start_date = as.POSIXct("2015-08-26 17:00","GMT")
end_date = as.POSIXct("2015-08-30 18:00","GMT")
...
# graphing stuff
#use one of the eggs df to get the time
tmp <- paste("dfMin_", feed_id,"_", v, sep = '')
timeStart <- paste(tmp,"$timeGMT[1]")
nrows <- nrow(eval(parse(text = tmp)))
timeEnd <- paste(tmp,"$timeGMT[",nrows,"]",sep="")
...
egg2comparison <- function(eggList,gas) {
  dfList <- c(
    ...
    paste("dfMin",eggList[5],gas,sep="_"),
    paste("dfMin",eggList[6],gas,sep="_"),
    paste("dfMin",eggList[7],gas,sep="_"))
  #   f.eggPlot <- function(s) {
  #    
  #     tmp <- paste("p <- p +  geom_line(data=as.data.frame(eval(parse(text = ", 
  #                  s, "))), aes(x = timeLocal, y = value, color=",
  #                  '"',s,'"',"))", sep='')
  #     eval(parse(text = tmp))
  #}
  p<- ggplot()
  #sapply(dfList, f.eggPlot)
  ...
  p <- p +  geom_line(data=as.data.frame(eval(parse(text = (dfList[5])))), 
                      aes(x = timeLocal, y = value, color=paste("egg",dfList[5],sep="")))
  p <- p +  geom_line(data=as.data.frame(eval(parse(text = (dfList[6])))), 
                      aes(x = timeLocal, y = value, color=paste("egg",dfList[6],sep="")))
  p <- p +  geom_line(data=as.data.frame(eval(parse(text = (dfList[7])))), 
                      aes(x = timeLocal, y = value, color=paste("egg",dfList[7],sep="")))
  p <- p + scale_colour_manual("", 
                               breaks = c(
                                 ...
                                 paste("egg",dfList[5],sep=""),
                                 paste("egg",dfList[6],sep=""),
                                 paste("egg",dfList[7],sep="")
                               ),
                               values = c("red", "green", "blue", "yellow","gray","lightskyblue","orange"))
...
dFMin.NO2.Aug26 <- c(dfMin_1058978669_NO2$timeLocal, 
                           dfMin_1058978669_NO2$value,
                           dfMin_9544854_NO2$value,
                           dfMin_109431716_NO2$value,
                           dfMin_2122830268_NO2$value,
                           dfMin_546125879_NO2$value,
                           dfMin_1203534876_NO2$value,
                           dfMin_58540724_NO2$value,
                           dfMin_1058978669_Temperature$value,
                           dfMin_9544854_Temperature$value, 
                           dfMin_109431716_Temperature$value,
                           dfMin_2122830268_Temperature$value,
                           dfMin_546125879_Temperature$value,
                           dfMin_1203534876_Temperature$value,
                           dfMin_58540724_Temperature$value)
...
egg5 <- paste("dfMin",AQE2aEggs[5],gas,sep="_") 
egg6 <- paste("dfMin",AQE2aEggs[6],gas,sep="_") 
egg7 <- paste("dfMin",AQE2aEggs[7],gas,sep="_") 
