...
o3_dataDir <- "CMUDec2017" # directory beneath data-raw where the o3 data file is located
# data from the official monitor comes in two files for the December testing
officialFileName1 <- "8dec17logfile.txt"
officialFileName2 <- "11dec17logfile.txt"
o3_official <- data.table(V1 = numeric(), V2 = character(),V3 = character(),V4 = integer(),V5 = character(),V6 = numeric())
for (i in c(officialFileName1, officialFileName2)) {
  o3_officialFileLoc <- paste("data-raw", o3_dataDir, i, sep = "/")
  temp <- as.data.table((read.table(o3_officialFileLoc, stringsAsFactors = FALSE, na.strings = "---")))
  o3_official <- rbind(o3_official, temp)
}

o3_official[, month :=  match(V3, month.abb)]
o3_official[, date := paste(V6, month, V4, sep = "-" )]
o3_official[, timeStamp := paste(date, V5, sep = " " )]
colsToDelete <- c("V2", "V3", "V4", "V5", "V6", "date",  "month")
o3_official[, (colsToDelete) := NULL]
...
setnames(o3_official, old = "V1", new = "o3_compVal_ppb_o3_official") #needed to have the correct number of columns later on.
...
for (i in 1:length(eggSerialList)) {
  ...
  if (eggType %in% "no2o3") {
    ...
    test <- as.data.table(predict.lm(fit.inSample, mergedDT.noNA.outSample, se.fit = TRUE))
  }
}
